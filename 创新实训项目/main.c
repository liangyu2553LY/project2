...
//定义WIFI控制命令
#define LED1_ON_CMD			'1'//开灯
#define LED1_OFF_CMD		'2'//关灯
#define amoxilin			  '3'
#define banlangen		    '4'
...
unsigned int chiyaoliang1=0;//吃药量1//板蓝根
unsigned int chiyaoliang2=0;//吃药量2//阿莫新林
...
void key_control()
{
	switch(keynum)
	{
		...
		case 2:
			{
				ESP8266_SendData("按时吃药中");	//通过串口发巳数据到APP
				//ES=0;
				keynum=0;
				led=1;
				//ES=1;
		
				timeflag=1;//按时吃药置1,关闭蜂鸣器和led灯
			}
			break;
//		case 3:
//			break;
		...
		case 9:        //调时--
		{
			...
			{
				timer2[2]--;
				if(timer2[2]==0){if(timer2[2]==85)timer2[2]=23;}
				addr_12864(4,5);
				write_dat(shuzi[timer2[2]/10]);
				write_dat(shuzi[timer2[2]%10]);
				keynum=0;
		...
		case 11:        //设置吃药量--
		{
			...
			{
				chiyaoliang1--;
				if(chiyaoliang1==0){chiyaoliang1=9;}
				addr_12864(3,3);
				write_dat(shuzi[chiyaoliang1]);
				keynum=0;
			}
			...
			{
				chiyaoliang2--;
				if(chiyaoliang2==0){chiyaoliang2=9;}
				addr_12864(3,3);
				write_dat(shuzi[chiyaoliang2]);
				keynum=0;
			}
